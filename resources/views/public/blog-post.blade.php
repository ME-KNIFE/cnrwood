...
@php
    $title           = ($post->getTranslation('meta_title', 'tr') ?? $post->getTranslation('title', 'tr')) . ' — CNRWOOD';
    $metaDescription = $post->getTranslation('meta_description', 'tr') ?? $post->getTranslation('excerpt', 'tr') ?? '';
@endphp
...
<section class="bg-[#F5F0E8] border-b border-[#E6DFD2]">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <nav class="text-sm text-[#8B5A2B] mb-3">
            ...
            <span class="mx-1">/</span>
            <span class="text-[#3E2006]">{{ \Illuminate\Support\Str::limit($post->getTranslation('title', 'tr'), 50) }}</span>
        </nav>
...
<article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <div class="prose prose-stone max-w-none
        ...
        {!! nl2br(e($post->getTranslation('body', 'tr'))) !!}
    </div>
</article>
